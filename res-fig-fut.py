...
ALL_Model = pd.read_excel('./data-fut/ALL Model.xls')
AGCM_CGCM = pd.read_excel('./data-fut/AGCM-CGCM.xls')
HR_LR = pd.read_excel('./data-fut/HR-LR.xls')
HR = pd.read_excel('./data-fut/HR.xls')
LR = pd.read_excel('./data-fut/LR.xls')
AGCM = pd.read_excel('./data-fut/AGCM.xls')
CGCM = pd.read_excel('./data-fut/CGCM.xls')
...
mks = [[True,True,True,False,False,True,True,False],
...
#             ax.scatter(j+(i-1)*wids,-1.45,marker='*',color='#1e90ff') #每个下面增加标记点
            ax.text(j+(i-1)*wids,-0.44,'Y',va='center',ha='center',fontsize=7,color='r',weight="bold",zorder=500)
        else:
            ax.text(j+(i-1)*wids,-0.44,'N',va='center',ha='center',fontsize=7,color='r',weight="bold",zorder=500)
...
ax.set_ylim(-0.4,0.4)#纵轴显示范围
ax.set_yticks(np.arange(-0.4,0.5,0.2))#纵轴刻度
...
#             ax.scatter(j+(i-1)*wids,-7.45,marker='*',color='#1e90ff') #每个下面增加标记点
            ax.text(j+(i-1)*wids,-2.25,'Y',va='center',ha='center',fontsize=7,color='r',weight="bold",zorder=500)
        else:
            ax.text(j+(i-1)*wids,-2.25,'N',va='center',ha='center',fontsize=7,color='r',weight="bold",zorder=500)
...
ax.set_ylim(-0.3,0.6)#纵轴显示范围
ax.set_yticks(np.arange(-0.3,0.7,0.3))#纵轴刻度
...
add_label(ax=ax,ticks=np.linspace(0.3,0.73,2),labels=[r"$\varphi_{LMI}$",r"fitted $\varphi_{LMI}$"],colors=plt.cm.RdPu(np.linspace(0.2,0.4,2)),wids=0.3)
ax.set_ylim(-0.3,0.6)
ax.set_yticks(np.arange(-0.3,0.7,0.3))
ax.plot([0.55,0.95],[1.25,1.25],transform=ax.transAxes,color='k',lw=0.8,clip_on=False)
#3行第2个图
ax = ax.inset_axes([1.2,0,3,1])
colors = ["#FFC0CB",'limegreen']
...
plt.savefig('fig-fut.png',dpi=300,bbox_inches='tight')